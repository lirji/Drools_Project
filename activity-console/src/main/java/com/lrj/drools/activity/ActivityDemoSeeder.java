...
/**
 * demo 种子：给「商品池自动圈选」与「选店铺→勾商品 picker」造点数据，让这两条绑定方式在浏览器里也能跑通
 * （否则 {@code demo_product}/{@code demo_store} 空表，picker 无店可选、autoBoundCount 恒为 0）。
 *
 * <p>仅当 {@code activity.marketing.seed-demo-data=true} 时启用（application.yml 里开、
 * 测试的 @TestPropertySource 一律关 → 不污染 {@code ActivityMarketingFlowTest} 的池断言）。
 *
 * <p><b>多租户</b>：前端 dev 默认租户是 {@code acme}，而后端 dev-default 是 {@code __dev__}——两者都需要目录数据，
...
public class ActivityDemoSeeder implements CommandLineRunner {
    ...
    private final DemoStoreRepository storeRepo;
    ...
    public void run(String... args) {
        // 启动期无请求上下文 → 逐个 demo 租户显式播种，让各租户的请求都能读到本租户目录数据。
    ...
    private void seedTenant(String tenant) {
        ...
        // store_id 与 demo_product.spu_id 一样是**全局业务键**（单列主键），跨租户不能复用同一个 id，
        ...
        if (tenant.equals(tenantProps.getDevDefault())) {
            ...
            storeRepo.save(new DemoStoreEntity(2, "折扣店", 1));
            ...
            productRepo.save(new DemoProductEntity(9201L, 2, "跑步鞋", "sports", new BigDecimal("260"), "hot", 1));
            productRepo.save(new DemoProductEntity(9202L, 2, "瑜伽垫", "sports", new BigDecimal("90"), "new", 1));
            ...
            log.info("[ActivityDemoSeeder] __dev__: 2 店(1/2) + 6 商品 + 商品池 poolId={}", pool.getId());
        } else {
            // acme（auth 档主租户）：store_id 用 3/4（全局唯一），造 2 店各 2 商品，不叠 pool
            storeRepo.save(new DemoStoreEntity(3, "acme 旗舰店", 1));
            storeRepo.save(new DemoStoreEntity(4, "acme 折扣店", 1));
            productRepo.save(new DemoProductEntity(8101L, 3, "智能手表", "electronics", new BigDecimal("899"), "hot", 1));
            productRepo.save(new DemoProductEntity(8102L, 3, "保温杯", "home", new BigDecimal("59"), "new", 1));
            productRepo.save(new DemoProductEntity(8201L, 4, "蓝牙音箱", "electronics", new BigDecimal("320"), "hot", 1));
            productRepo.save(new DemoProductEntity(8202L, 4, "双肩包", "bags", new BigDecimal("199"), "cheap", 1));
            log.info("[ActivityDemoSeeder] {}: 2 店(3/4) + 4 商品（无商品池）", tenant);
        }
    }
}
