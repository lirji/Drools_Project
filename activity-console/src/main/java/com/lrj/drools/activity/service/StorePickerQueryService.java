...
import com.lrj.drools.activity.persistence.DemoProductRepository.StoreProductCount;
 ...
 * 这里回答「当前租户有哪些店 / 某店有哪些在架商品可勾选」，数据源是 {@code demo_product}(+{@code demo_store} 供名)。
...
public class StorePickerQueryService {
    ...
    public StorePickerQueryService(DemoProductRepository productRepo, DemoStoreRepository storeRepo) {
        this.productRepo = productRepo;
        this.storeRepo = storeRepo;
    }

    /**
     * 店铺列表（Arch B）：由 demo_product 的 group by store_id 驱动（只列有在架商品的店），
    ...
    public List<PickerStore> stores() {
        ...
        Map<Integer, DemoStoreEntity> byId = storeIds.isEmpty() ? Map.of()
                : storeRepo.findAllById(storeIds).stream()
                    .collect(Collectors.toMap(DemoStoreEntity::getStoreId, s -> s, (a, b) -> a));
        return agg.stream()
                .map(r -> {
                    DemoStoreEntity s = byId.get(r.getStoreId());
    ...
    public PickerProductPage products(Integer storeId, String keyword, int page, int size) {
        ...
        Page<DemoProductEntity> pageResult = productRepo.pageStoreProducts(storeId, kw, pageable);
    ...
    /** 店铺一行：{@code storeName} 可空（demo_store 里查不到时回退，前端显示「店铺 #id」）。 */
