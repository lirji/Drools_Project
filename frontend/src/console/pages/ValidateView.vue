  ...
  <section data-testid="validate-view">
    ...
    <div class="validate-grid">
      ...
      <section class="result-card" :aria-busy="busy">
        ...
        <!-- 换代前这里是自绘的 470px 呼吸块（全站第 2 套 loading）。现在统一走 Skeleton，
             外壳保留 role=status + aria-live 以免丢掉屏幕阅读器的"正在决策"播报。 -->
        <div v-if="busy" class="loading-result" role="status" aria-live="polite">
          <span><Icon name="workflow" :size="24" /></span><h3>规则引擎正在决策</h3><p>正在筛选有效活动、计算资格条件并应用合并策略…</p>
          <Skeleton :rows="3" />
        </div>
...
<style scoped>
.mode-picker { display: grid; grid-template-columns: 1fr 1fr; gap: var(--sp-3); margin-bottom: var(--sp-4); }.mode-picker > button { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; align-items: center; gap: var(--sp-3); min-width: 0; padding: var(--sp-3) var(--sp-4); border: 1px solid var(--border); border-radius: var(--radius-lg); background: var(--bg-elev); color: var(--text); cursor: pointer; text-align: left; box-shadow: var(--shadow-sm); }.mode-picker > button:hover { border-color: var(--border-strong); }.mode-picker > button:disabled { cursor: wait; opacity: .7; }.mode-picker > button.active { border-color: var(--accent); background: linear-gradient(100deg, var(--accent-soft), var(--bg-elev)); box-shadow: 0 0 0 1px var(--accent-line); }.mode-icon { display: inline-flex; align-items: center; justify-content: center; width: 42px; height: 42px; border-radius: 12px; background: var(--accent-soft); color: var(--accent); }.mode-icon.gifts { background: var(--blue-soft); color: var(--blue); }.mode-picker small, .mode-picker strong, .mode-picker i { display: block; }.mode-picker small { color: var(--accent); font-size: var(--fs-2xs); font-style: normal; letter-spacing: .08em; }.mode-picker strong { margin-top: 2px; font-size: var(--fs-sm); }.mode-picker i { overflow: hidden; margin-top: 2px; color: var(--text-faint); font-size: var(--fs-2xs); font-style: normal; text-overflow: ellipsis; white-space: nowrap; }.choice { display: inline-flex; align-items: center; justify-content: center; width: 20px; height: 20px; border: 1px solid var(--border-strong); border-radius: 50%; }.active .choice { border-color: var(--accent); background: var(--accent); color: var(--text-invert); }
.validate-grid { display: grid; grid-template-columns: minmax(0, .9fr) minmax(0, 1.1fr); gap: var(--sp-4); align-items: stretch; }.context-card, .result-card { min-width: 0; overflow: hidden; border: 1px solid var(--border); border-radius: var(--radius-lg); background: var(--bg-elev); box-shadow: var(--shadow-sm); }.card-head { display: flex; align-items: center; justify-content: space-between; min-height: 72px; padding: var(--sp-3) var(--sp-4); border-bottom: 1px solid var(--border); background: var(--bg-soft); }.card-head > div { display: flex; align-items: center; gap: var(--sp-3); }.step { display: inline-flex; align-items: center; justify-content: center; width: 34px; height: 34px; border-radius: 9px; background: var(--accent-soft); color: var(--accent); font-size: var(--fs-xs); font-weight: var(--fw-bold); font-variant-numeric: tabular-nums; }.card-head h2 { margin: 0; font-size: var(--fs-md); }.card-head p { margin: 1px 0 0; color: var(--text-faint); font-size: var(--fs-2xs); }.example { display: inline-flex; align-items: center; gap: var(--sp-1); padding: var(--sp-2); border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--bg-elev); color: var(--accent); cursor: pointer; font: inherit; font-size: var(--fs-xs); }
.form-section { padding: var(--sp-4); border-bottom: 1px solid var(--border); }.section-label { display: flex; align-items: center; justify-content: space-between; margin-bottom: var(--sp-3); }.section-label span { font-size: var(--fs-xs); font-weight: var(--fw-bold); letter-spacing: .06em; text-transform: uppercase; }.section-label small { color: var(--text-faint); font-size: var(--fs-2xs); }.field-grid { display: grid; grid-template-columns: 1fr 1fr; gap: var(--sp-3); }.field { display: flex; flex-direction: column; gap: var(--sp-1); min-width: 0; }.field.full + .field-grid, .field-grid + .field.full { margin-top: var(--sp-3); }.field > span { display: flex; justify-content: space-between; color: var(--text-soft); font-size: var(--fs-xs); font-weight: var(--fw-medium); }.field > span i { color: var(--text-faint); font-size: var(--fs-2xs); font-style: normal; font-weight: var(--fw-medium); }.input-wrap { display: flex; align-items: center; gap: var(--sp-2); min-height: 40px; padding: 0 var(--sp-3); border: 1px solid var(--border-ctl); border-radius: var(--radius-sm); background: var(--bg-soft); color: var(--text-faint); }.input-wrap:focus-within { border-color: var(--accent); background: var(--bg-elev); box-shadow: var(--focus-ring); }.input-wrap input { width: 100%; min-width: 0; border: 0; outline: 0; background: transparent; color: var(--text); font: inherit; font-size: var(--fs-sm); }.input-wrap b { color: var(--text-soft); font-size: var(--fs-sm); }.actions { display: flex; gap: var(--sp-2); justify-content: flex-end; padding: var(--sp-4); }.actions button { min-height: 42px; border-radius: var(--radius-sm); cursor: pointer; font: inherit; font-size: var(--fs-xs); }.actions button:disabled { cursor: wait; opacity: .6; }.clear { padding: 0 var(--sp-4); border: 1px solid var(--border); background: var(--bg-elev); color: var(--text-soft); }.run { display: inline-flex; align-items: center; justify-content: center; gap: var(--sp-2); min-width: 170px; padding: 0 var(--sp-4); border: 0; background: linear-gradient(100deg, var(--accent), var(--accent-2)); color: var(--text-invert); font-weight: var(--fw-semibold); box-shadow: 0 8px 18px color-mix(in srgb, var(--accent) 20%, transparent); }.gift-run { background: linear-gradient(100deg, var(--blue), var(--accent-2)); }.run.secondary { border: 1px solid var(--border); background: var(--bg-elev); color: var(--text-soft); box-shadow: none; }.spinning { animation: spin .9s linear infinite; }
.loading-result { display: flex; min-height: 470px; flex-direction: column; align-items: center; justify-content: center; padding: var(--sp-6); text-align: center; }.loading-result > span { display: inline-flex; align-items: center; justify-content: center; width: 58px; height: 58px; border-radius: 18px; background: var(--accent-soft); color: var(--accent); animation: breathe 1s ease-in-out infinite alternate; }.loading-result h3 { margin: var(--sp-3) 0 var(--sp-1); font-size: var(--fs-md); }.loading-result p { max-width: 360px; margin: 0; color: var(--text-faint); font-size: var(--fs-xs); }.loading-result > div { width: min(280px, 80%); margin-top: var(--sp-5); }.loading-result i { display: block; height: 7px; margin-top: var(--sp-2); border-radius: var(--radius-pill); background: var(--bg-soft); }.loading-result i:nth-child(2) { width: 80%; }.loading-result i:nth-child(3) { width: 60%; }.result-error { display: flex; min-height: 110px; flex-direction: column; justify-content: center; margin: var(--sp-4); }.result-error span { margin-top: 2px; font-size: var(--fs-xs); }
.decision-summary { display: grid; grid-template-columns: auto minmax(0, 1fr) auto; align-items: center; gap: var(--sp-3); margin: var(--sp-4); padding: var(--sp-4); border: 1px solid var(--border); border-radius: var(--radius-lg); }.decision-summary.hit { border-color: color-mix(in srgb, var(--green) 28%, var(--border)); background: var(--green-soft); }.decision-summary.miss { background: var(--bg-soft); }.decision-icon { display: inline-flex; align-items: center; justify-content: center; width: 46px; height: 46px; border-radius: 14px; background: var(--bg-elev); color: var(--green); }.miss .decision-icon { color: var(--text-faint); }.decision-summary small { color: var(--green); font-size: var(--fs-2xs); letter-spacing: .08em; }.decision-summary h3 { margin: 2px 0; font-size: var(--fs-md); }.decision-summary p { margin: 0; color: var(--text-soft); font-size: var(--fs-2xs); }.hit-amount { color: var(--green); font-size: 20px; font-variant-numeric: tabular-nums; }.result-metrics { display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--sp-2); margin: 0 var(--sp-4) var(--sp-4); }.result-metrics article { min-width: 0; padding: var(--sp-3); border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--bg-soft); }.result-metrics small, .result-metrics strong { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.result-metrics small { color: var(--text-faint); font-size: var(--fs-2xs); }.result-metrics strong { margin-top: 2px; font-size: var(--fs-xs); }.mono { font-family: var(--mono); }
.gift-list { display: flex; flex-direction: column; }.gift-row { display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: var(--sp-2); padding: var(--sp-2) 0; border-bottom: 1px solid var(--border); }.gift-row:last-child { border-bottom: 0; }.gift-row > span { display: inline-flex; align-items: center; justify-content: center; width: 26px; height: 26px; border-radius: 7px; background: var(--bg-soft); color: var(--text-faint); font-size: var(--fs-2xs); font-variant-numeric: tabular-nums; }.gift-row strong, .gift-row small { display: block; }.gift-row strong { font-size: var(--fs-xs); }.gift-row small { color: var(--text-faint); font-size: var(--fs-2xs); }.gift-row b { color: var(--text-soft); font-size: var(--fs-2xs); font-variant-numeric: tabular-nums; }
.trace-panel { overflow: hidden; margin: var(--sp-4); border: 1px solid var(--border); border-radius: var(--radius-sm); }.trace-head { display: flex; align-items: center; justify-content: space-between; padding: var(--sp-2) var(--sp-3); border-bottom: 1px solid var(--border); background: var(--bg-soft); }.trace-head span { display: inline-flex; align-items: center; gap: var(--sp-2); font-size: var(--fs-xs); font-weight: var(--fw-semibold); }.trace-head small { color: var(--text-faint); font-size: var(--fs-2xs); font-variant-numeric: tabular-nums; }.traces { padding: var(--sp-2) var(--sp-3); }.trace-row { display: grid; grid-template-columns: 34px 1fr; gap: var(--sp-2); min-height: 36px; }.trace-row > span { position: relative; color: var(--accent); font-size: var(--fs-2xs); font-variant-numeric: tabular-nums; }.trace-row > span::after { content: ''; position: absolute; top: 15px; bottom: -4px; left: 3px; width: 1px; background: var(--border); }.trace-row:last-child > span::after { display: none; }.trace-row > span i { display: inline-block; width: 7px; height: 7px; margin-right: 5px; border-radius: 50%; background: var(--accent); box-shadow: 0 0 0 3px var(--accent-soft); }.trace-row p { margin: 0; color: var(--text-soft); font-size: var(--fs-2xs); line-height: 1.55; }.no-trace { padding: var(--sp-4); color: var(--text-faint); font-size: var(--fs-xs); text-align: center; }
