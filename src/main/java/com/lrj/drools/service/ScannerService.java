...
public class ScannerService {
    ...
    private int generation = 0;          // 每次 deploy +1, 标记当前 live 的内容代次
    ...
    public synchronized DeployResult deploy(String drl) {
        ...
        kfs.generateAndWritePomXML(releaseId);   // KJAR 自带 pom.xml + pom.properties
        ...
        repo.installArtifact(releaseId, kieModule, pomFile());

        generation++;

        // 4. 首次创建 container + scanner; 之后 scanNow 热替换
        String action;
        if (container == null) {
            ...
            scanner.scanNow();   // ← KieScanner 重新解析 SNAPSHOT, 命中新内容则替换 KieBase
    ...
    private File pomFile() {
        try {
            ...
            f.deleteOnExit();
            Files.writeString(f.toPath(), KieBuilderImpl.generatePomXml(releaseId));
            return f;
        } catch (IOException e) {
