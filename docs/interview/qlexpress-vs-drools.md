...
> 我做过一个**多租户活动引擎**：活动规则不写在代码里，而是**规则即数据**存在 DB（活动 / 规则 / 条件 / 发布代际几张表）。
> 一开始整条决策链路都走 Drools——`ActivityDrlBuilder` 把配置翻译成 DRL 文本，`KieHelper` 运行时编译成 KieBase，`StatelessKieSession` + global 收集结果。
> **后来把大部分移出了规则引擎**：判据是「这条规则需不需要*其它规则的结论*」——阶梯落档是标量分段函数、折扣合并是一次 reduce、资格条件树是单事实布尔谓词，三者都不需要，于是改纯 Java 直接求值。
> 现在生产上真正执行 DRL 的只剩**买赠**一条通道，外加写平面的编译校验与发布预热。这个决策我做了实测：同一份负载下每活动常驻足迹 **纯 Java 1.8 KB / QLExpress 8.4 KB / Drools ~180 KB**，决策 CPU 差 67 倍——**差的是量级，不是调优空间**。
>
> 工程化上做了四件事：
