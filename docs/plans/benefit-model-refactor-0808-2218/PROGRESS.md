...
| | 改造前 | 现在 |
|---|---|---|
| activity-common | 63 | **87**（+24；折扣数学 20 例） |
| activity-console | 40 | **162**（+122；金标集在 Java 与全 Drools 两条路各跑一遍；PR-5 批量版本正确性 +2；折扣型金标 7×2 + 写平面 7 + 回退 4） |
| activity-decision | 12 | **17**（+5） |
| 前端 Vitest | 62 | **150**（+88；PR-5 的 benchModel 20 + ListView 5，PR-6 的 playbooks 11 + 模板屏 5 + 编辑器 8） |
| **端到端金额断言** | **7** | **48**（金标集，含折扣型 7 例） |
...
其提出的 4 条由我逐条自查（读路径 fail-open 已修，另 3 条是前端未跟上，本次一并做掉），
但 **`parity` / `data` / `edge` 三个角度从未执行**。下次动这块之前应补跑：
DRL 与 Java 的 salience 交互、快照/迁移、以及 STACK 下多张折扣券累加是否可能超过订单金额。
