...
| POST | `/fraud/patterns`       | Step 8 扩展：orders + logins 两条流合并成时间线，演示 `window:length` / `after[..]` 时序 / 多 entry-point，返回 `FraudAlert[]`（type = LENGTH_BURST / PROBE_THEN_STRIKE / FAST_ORDER_AFTER_LOGIN） |
...
- Step 13：DRL 里也可以用 `?queryName(args;)` 在规则 LHS 触发后向链做"pull-driven 规则"——`/backward/contains` 走 Java API 路径 (getQueryResults) 更直观, **扩展 `/backward/derive` 才演示 `?query` 的 LHS 形态**: 插一个 driver fact (`WatchTarget`), 规则 LHS 用 `?isContainedIn($thing, $zone;)` 反向证明, 成立才 fire。同一个 query 两种范式共用
...
- Step 19：`forall` **空集为真** (vacuous truth) —— `forall(OrderItem(...))` 在一条 OrderItem 都没有时也成立, 空购物车会被误判"全部合规"。本 demo 前置一条 `exists OrderItem()` 兜底。两模式 `forall($i: 基准 附加约束)` 用 `this == $i` 锁到同一条 fact, 是官方 Bus 示例的标准写法
- Step 19：`eval(任意布尔)` 是 LHS 最贵的元素——**不进 RETE 字段索引, 每次相关 fact 变化整段重算**。只在"确实无法索引化"时用 (本 demo 判"跨 fact 聚合总额 > 按 VIP 等级动态算的阈值"); 能写成 `Customer(vipLevel >= 2)` 这种字段约束或 join 的, 永远别退化成 eval。DRL `function reviewThreshold(int)` 是包级辅助方法, LHS/RHS 都能调
...
- Step 22：`fireUntilHalt` **阻塞不返回**，必须跑在单独线程，否则请求线程卡死。停止只能靠 `session.halt()`(可跨线程调，跟 Step 14 一样)。本 demo 用哨兵任务 + 低 salience 的 halt 规则让引擎处理完所有任务后自己收尾，比主线程直接 halt 更确定(避免异步竞态漏处理)。`session.getObjects()` 捞结果前要先 join 那个 fire 线程
