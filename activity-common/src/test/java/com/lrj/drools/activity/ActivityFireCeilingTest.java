...
class ActivityFireCeilingTest {
    ...
    private static final List<EligibilityRuleDef> DEFS =
            List.of(new EligibilityRuleDef("actFire", "numberAttr(\"orderAmount\") >= 1"));

    private ActivityRuleContext ctxWithEligibleCandidate() {
        ...
        c.setEligible(true);
        c.setRedPackageAmount(new BigDecimal("50"));
        ctx.addCandidate(c);
        return ctx;
    }
    ...
    void generousCeiling_engineRunsAndReturns() {
        ...
        ActivityRuleResult result = svc.evalEligibility(ctxWithEligibleCandidate(), DEFS);
