...
ensure_spa_app(){
  ...
  if [ -n "${exist}" ]; then
    ...
    [ "${st}" = "ok" ] && ok "app ${cid} 已存在 → 更新 redirectUris 并入网关/SPA/dev 回调（幂等追加）" \
  ...
  [ "${st}" = "ok" ] && ok "创建 SPA app ${cid}（org=${org}, grant=authorization_code+refresh, redirects=legacy+gateway+spa+dev）" \
